...
class Map:
    class Pose:
        def __init__(self, 
                     ...
                     origin: Optional['Map.Pose']=None, # Frame of reference from which x-y coordinates are based
            ...
            if self.hasGPSCoords() and self.origin is not None:
                origin = cast(Map.Pose, self.origin)
        ...
        def changeOrigin(self, new_origin: 'Map.Pose', copy: Optional[bool] = False):
            if not new_origin.isValidOrigin():
                return None
            # Do the conversion via Cartesian transformations first
            transform_vector = Map.Pose(lat=new_origin.lat, lon=new_origin.lon, heading=new_origin.heading, origin=self.origin, yaw=None)
            ...
            if self.hasGPSCoords():
                testPose = Map.Pose(self.lat, self.lon,x_new,y_new,new_origin,self.heading, yaw_new)

        def copy_from(self, other: 'Map.Pose'):
            """
            ...
            """
            if not isinstance(other, Map.Pose):
